...
void gl_save_framebuffer(bm_rgba_t* bm);
...
glsha_t create_texture_shader()
{
    ...
    glsha_t sha = gl_shader_create(vsh, fsh, 2, ((const char*[]){"position", "texcoord"}), 13, ((const char*[]){"projection", "sampler[0]", "sampler[1]", "sampler[2]", "sampler[3]", "sampler[4]", "sampler[5]", "sampler[6]", "sampler[7]", "sampler[8]", "sampler[9]", "domask", "texdim"}));
...
glsha_t create_color_shader()
{
    ...
    return gl_shader_create(vsh, fsh, 2, ((const char*[]){"position", "texcoord"}), 1, ((const char*[]){"projection"}));
}
...
glsha_t create_blur_shader()
{
    ...
    return gl_shader_create(vsh, fsh, 2, ((const char*[]){"position", "texcoord"}), 2, ((const char*[]){"projection", "samplera"}));
}
...
void gl_draw_framebuffer_in_framebuffer(int src_page, int tgt_page, glrect_t src_reg, glrect_t tgt_reg, glrect_t window, gl_sha_typ_t shader)
{
    glUseProgram(gl.shaders[shader].name);
